...
impl CubicRealPolynomial {
    pub fn compute_real_roots(a: f64, b: f64, c: f64, d: f64) -> Option<Vec<f64>> {
        let roots;
        let ratio;
        if a == 0.0 {
            ...
            if c == 0.0 {
                ...
                // x * (a * x^2 + c) = 0.
                roots = QuadraticRealPolynomial::compute_real_roots(a, 0., c);
            ...
            return compute_real_roots(a, 0., c, d);
        } else if c == 0.0 {
            if d == 0.0 {
                // x^2 * (a * x + b) = 0.
                ratio = -b / a;
            ...
            // a * x^3 + b * x^2 + d = 0.
            return compute_real_roots(a, b, 0., d);
        } else if d == 0.0 {
            // x * (a * x^2 + b * x + c) = 0
            roots = QuadraticRealPolynomial::compute_real_roots(a, b, c);
            if roots.is_some() {
                let roots = roots.unwrap();
                if roots.len() == 0 {
                    return Some(vec![0.0]);
        ...
        return compute_real_roots(a, b, c, d);
    }
}

fn compute_real_roots(a: f64, b: f64, c: f64, d: f64) -> Option<Vec<f64>> {
    let A = a;
    let B = b / 3.0;
    let C = c / 3.0;
    let D = d;
